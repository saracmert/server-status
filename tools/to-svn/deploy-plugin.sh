...
ASSETS_DIR="assets"              # the name of the assets directory that you are using
POT_DIR="languages/"                     # name of your language file directory
SVNUSER="extendwings"                          # your svn username
...
NEWVERSION2=`awk -F': ' '/^.*Version.*/{print $2}' ./server-status.php | tr -d '\r'`
...
TEXTDOMAIN=`awk -F': ' '/^.*Text Domain.*/{print $2}' ./server-status.php | tr -d '\r'`
...
POT_DIR=`awk -F': ' '/^.*Domain Path.*/{print $2}' ./server-status.php | tr -d '\r'`
if [ -z "$POT_DIR" ]; then
